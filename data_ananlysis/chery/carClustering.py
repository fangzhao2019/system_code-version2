...
def loadEffectiveCar():
    conn = pymysql.connect(host='47.99.116.136',user='root',passwd='3H1passwd',port=3306,db='car_test',charset='utf8')
    cursor=conn.cursor()
    sql_sentence='select car_id,associated_id from car_contend_value'
    cursor.execute(sql_sentence)
    results = cursor.fetchall()
    carCandidate=[]
    for r in results:
        carCandidate.append([r[0],r[1]])
    effectiveCar=[]
    targetCar=[2485,2492,2827,2398,2499]
    for x in range(len(carCandidate)):
        car1=carCandidate[x][0]
        car2=carCandidate[x][1]
        if not car1 in targetCar:
            if not car2 in targetCar:
                continue
        effectiveCar.append(car1)
        effectiveCar.append(car2)  
    return list(set(effectiveCar))

def loadCarCount():
    effectiveCar=loadEffectiveCar()
    ...
    carCount={}
    for r in results:
        if r[0] in effectiveCar:
            carCount[r[0]]=r[1]
    return carCount

def carFilter(carCount,carSubSet):
    dataSet=[]
    for car_id in carSubSet:
        #if carCount[car]>10:
        if car_id in carCount.keys():
            dataSet.append(car_id)
    return(dataSet)
...
def createRelationMat(carSubSet,carSubRelation):
    ...
    for i in range(len(carSubSet)):
        for j in range(len(carSubSet)):
            ...
            else:relationMat[i,j]=(relation-pmin)/float(pmax-pmin+1)
    return relationMat
...
for car_type_id in type_unique_value:
    ...
    if len(carSubRelation)==0:continue
    carSubSet1=[exa[0] for exa in carSubRelation]
    carSubSet2=[exa[1] for exa in carSubRelation]
    carSubSet=list(set(set(carSubSet1)|set(carSubSet2)))
    ...
    #聚类
    if len(carSubSet)<=10:
        n_cluster=int(len(carSubSet)/4)+1
    elif len(carSubSet)<=50:
        n_cluster=int(len(carSubSet)/6)+1
    else:
        n_cluster=int(len(carSubSet)/12)+1
